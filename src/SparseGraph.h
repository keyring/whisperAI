...
public:
  ...
  public:
    EdgeIterator(SparseGraph<node_type, edge_type> &graph, int node):G(graph), m_nodeIndex(node){
  ...
  public:
    ConstEdgeIterator(const SparseGraph<node_type, edge_type> &graph, int node):G(graph), m_nodeIndex(node){
...
template <class node_type, class edge_type>
const edge_type &SparseGraph<node_type, edge_type>::GetEdge(int src, int dst)const{
  
  assert((src < m_nodes.size()) && 
	  (src >= 0) &&
	  m_nodes[src].GetIndex() != INVALID_NODE_INDEX &&
	 "<SparseGraph::GetEdge>: invalid 'src' index");

  assert((dst < m_nodes.size()) &&
	  (dst >= 0) &&
	  m_nodes[dst].GetIndex() != INVALID_NODE_INDEX &&
...
template <class node_type, class edge_type>
edge_type &SparseGraph<node_type, edge_type>::GetEdge(int src, int dst){
...
void SparseGraph<node_type, edge_type>::AddEdge(EdgeType edge){
  ...
  if((m_nodes[edge.GetDst()].GetIndex() != INVALID_NODE_INDEX) &&
	...
	newEdge.SetDst(edge.GetSrc());
	newEdge.SetSrc(edge.GetDst());
...
void SparseGraph<node_type, edge_type>::SetEdgeCost(int src, int dst, double cost){
  ...
  assert(((unsigned int)src < m_nodes.size()) && ((unsigned int)dst < m_nodes.size()) &&
